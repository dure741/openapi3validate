...
module Openapi3Validate
  ...
  def self.api_validate(request_method, request_url, request_body_str, swagger_spec_path)
    str, strptr = validate(request_method, request_url, request_body_str, swagger_spec_path)
    release(strptr)
    str
  end
end
